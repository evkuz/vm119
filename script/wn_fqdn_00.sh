#!/bin/bash
  IP=$(echo `/sbin/ip a` | sed -n -e 's/^.*brd //p' | cut -d' ' -f1)
   ...
   HOSTNAME=vm`echo $SUBN`_`echo $NUM` /bin/hostname `echo $FQDN` echo "$IP" "$FQDN" "$HOSTNAME"
>> /etc/hosts
echo "HOSTNAME"=$HOSTNAME >> /etc/sysconfig/network Note: add the script for autostart: /etc$ cat
